...
def get_webb_psf_dims(zfile,osf):
    """Get the x and y pixel dimensions of a single PSF generated by WebbPSF

    PSFs made by WebbPSF have a specific structure. We'll need to know the dimensions of the PSFs for later use.

    Parameters
    ----------
    zfile : str 
        The path/filename of the FITS file containing the WebbPSF PSFs.
    osf : float
        The oversampling factor of the array.

    Returns
    -------
    tuple
        The x and y dimensions of the PSF in the form (x,y).
    """
...
def Psf_orig(wl, fnum, delta, nzero = 4, shape='airy'):
  '''
  Calculates an Airy Point Spread Function arranged as a data-cube. The spatial axies are
  0 and 1. The wavelength axis is 2. Each PSF area is normalised to unity.
